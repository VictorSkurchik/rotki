...
internal class AndroidPairingStartupReconciler(
    ...
    suspend fun reconcile(): AndroidPairingStartupState = when (safeReadJournal()) {
        PairingCleanupJournalReadOutcome.CleanupRequired ->
            AndroidPairingStartupState.CLEANUP_REQUIRED
        PairingCleanupJournalReadOutcome.Unavailable -> AndroidPairingStartupState.FAIL_CLOSED
        PairingCleanupJournalReadOutcome.Clear -> reconcileMaterial()
    }
    ...
    private suspend fun reconcileMaterial(): AndroidPairingStartupState {
        ...
        return when (record) {
            is PairingRecordReadOutcome.Present -> when (key) {
                is DeviceProofPublicKeyOutcome.PublicKey -> AndroidPairingStartupState.PAIRED
                DeviceProofPublicKeyOutcome.PairingRequired -> markCleanup()
                DeviceProofPublicKeyOutcome.UnexpectedFailure ->
                    AndroidPairingStartupState.FAIL_CLOSED
            }
            PairingRecordReadOutcome.Missing -> when (key) {
                is DeviceProofPublicKeyOutcome.PublicKey -> markCleanup()
                DeviceProofPublicKeyOutcome.PairingRequired ->
                    AndroidPairingStartupState.UNPAIRED
                DeviceProofPublicKeyOutcome.UnexpectedFailure ->
                    AndroidPairingStartupState.FAIL_CLOSED
            }
            PairingRecordReadOutcome.Corrupt -> when (key) {
            ...
            PairingRecordReadOutcome.Unavailable -> AndroidPairingStartupState.FAIL_CLOSED
        }
    }
    ...
    private suspend fun safeReadJournal(): PairingCleanupJournalReadOutcome = try {
        readJournal()
    ...
    private suspend fun safeReadRecord(): PairingRecordReadOutcome = try {
        readRecord()
    } catch (cancellation: CancellationException) {
        throw cancellation
    } catch (_: Exception) {
    ...
    private suspend fun safeReadCurrentKey(): DeviceProofPublicKeyOutcome = try {
        readCurrentKey()
    } catch (cancellation: CancellationException) {
        throw cancellation
    } catch (_: Exception) {
    ...
    private suspend fun safeMarkCleanupRequired(): PairingCleanupJournalWriteOutcome = try {
        markCleanupRequired()
    } catch (cancellation: CancellationException) {
        throw cancellation
    } catch (_: Exception) {
...
internal fun AndroidPairingStartupState.createStartupFacade(): CompanionFacade = when (this) {
...
internal suspend fun AndroidPairingStartupState.applyRetryTo(
    ...
    AndroidPairingStartupState.CLEANUP_REQUIRED -> retryCleanup()
    AndroidPairingStartupState.UNPAIRED -> {
        facade.unpair()
        PairingConnectionOutcome.NO_PENDING_PAIRING
    }
    AndroidPairingStartupState.PAIRED -> PairingConnectionOutcome.NO_PENDING_PAIRING
    AndroidPairingStartupState.FAIL_CLOSED ->
