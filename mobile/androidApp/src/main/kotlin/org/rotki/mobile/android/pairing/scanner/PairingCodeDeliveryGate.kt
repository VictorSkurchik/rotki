...
internal class PairingCodeDeliveryGate {
    ...
    fun activate(): Unit = synchronized(lock) {
        generation += 1L
        active = true
    }
    ...
    fun currentSession(): Long? = synchronized(lock) {
    ...
    fun fail(session: Long, deliver: () -> Unit): Unit {
        if (claim(session)) deliver()
    }

    fun restart(): Unit = synchronized(lock) {
        generation += 1L
        delivered = false
    }

    private fun claim(session: Long): Boolean = synchronized(lock) {
