...
class AndroidDeviceProofSignerTest {
    @Test
    fun `current and sign never create a missing key`() = runBlocking {
    ...
    @Test
    fun `create is the only creation path and returns a reusable X963 key`() = runBlocking {
        val keyStore = FakeDeviceSigningKeyStore()
        val signer = signer(keyStore)
    ...
    @Test
    fun `sign hashes the unchanged transcript once and returns protocol P1363`() = runBlocking {
    ...
    @Test
    fun `native and encoding failures map to typed outcomes`() = runBlocking {
        val currentFailure = FakeDeviceSigningKeyStore().apply {
        ...
        val invalidSignature = FakeDeviceSigningKeyStore().apply {
            createOrCurrent()
        ...
        val deleteFailure = FakeDeviceSigningKeyStore().apply {
            failure = IllegalStateException("native failure")
        }
    ...
    @Test
    fun `delete is idempotent and cancellation is never downgraded`(): Unit {
        runBlocking {
            val keyStore = FakeDeviceSigningKeyStore().apply { createOrCurrent() }
            val signer = signer(keyStore)
    ...
    private class FakeDeviceSigningKeyStore(
        ...
        private fun material(): AndroidSigningKeyMaterial =
            AndroidSigningKeyMaterial(keyPair.private, keyPair.public)
    }

    private companion object {
        fun p256KeyPair(): KeyPair = KeyPairGenerator.getInstance("EC").run {
