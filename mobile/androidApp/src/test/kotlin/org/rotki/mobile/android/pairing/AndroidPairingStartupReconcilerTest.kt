...
class AndroidPairingStartupReconcilerTest {
    @Test
    fun `paired root is restored only when record and key are both present`() = runTest {
        val fixture = Fixture(
    ...
    @Test
    fun `fully absent material starts unpaired without a cleanup marker`() = runTest {
        val fixture = Fixture()
    ...
    @Test
    fun `record without key is journaled before shared cleanup`() = runTest {
    ...
    @Test
    fun `key without record is journaled before shared cleanup`() = runTest {
    ...
    @Test
    fun `durable journal bypasses material reads and requires cleanup`() = runTest {
    ...
    @Test
    fun `unavailable journal or marker write remains fail closed`() = runTest {
        assertEquals(
    ...
    @Test
    fun `unavailable material reads remain fail closed without destructive intent`() = runTest {
        listOf(
    ...
    @Test
    fun `unknown startup authority uses DeviceLocked until retry proves a state`() = runTest {
        ...
        val pairedOutcome = AndroidPairingStartupState.PAIRED.applyRetryTo(facade) {
            error("No cleanup expected")
        }
        assertEquals(PairingConnectionOutcome.NO_PENDING_PAIRING, pairedOutcome)
        assertEquals(CompanionRootState.DeviceLocked, facade.status.value.rootState)

        val unpairedOutcome = AndroidPairingStartupState.UNPAIRED.applyRetryTo(facade) {
            error("No cleanup expected")
        }
    ...
    @Test
    fun `cleanup-required retry delegates once and failure stays fail closed`() = runTest {
        val facade = AndroidPairingStartupState.CLEANUP_REQUIRED.createStartupFacade()
        var cleanupCalls = 0

        val outcome = AndroidPairingStartupState.CLEANUP_REQUIRED.applyRetryTo(facade) {
            cleanupCalls += 1
    ...
    private class Fixture(
        ...
        suspend fun reconcile(): AndroidPairingStartupState = AndroidPairingStartupReconciler(
            readJournal = { journal },
            readRecord = {
                recordReads += 1
                record
...
private fun record(): PairingRecord {
    val origin = (EngineOrigin.parse("https://rotki.example") as
        EngineOriginParseOutcome.Accepted).origin
    val sessionId = (DeviceSessionId.parse(
        "AAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8",
    ) as ProtocolValueParseOutcome.Accepted<DeviceSessionId>).value
    return PairingRecord(origin, sessionId)
}

private fun publicKey(): X963PublicKey = (X963PublicKey.parse(
    "BGsX0fLhLEJH-Lzm5WOkQPJ3A32BLeszoPShOUXYmMKWT-NC4v4af5uO5-tKfA-eFivOM1drMV7Oy7ZAaDe_UfU",
) as ProtocolValueParseOutcome.Accepted<X963PublicKey>).value
