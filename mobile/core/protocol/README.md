...
The generic Ktor execution and platform-engine boundary lives in `:core:network`. Pairing QR and
registration DTOs plus Pairing-specific request construction live in `:feature:pairing:data`;
deferred challenge/proof, Access Session, rename, and revoke DTOs remain in `:shared`. This module
has no project, Ktor, database, Compose, Koin, native UI, or platform implementation dependency. The
