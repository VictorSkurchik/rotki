...
SHARED_DIR: Final = Path(__file__).resolve().parents[1]
...
FIXTURES_KOTLIN_PATH: Final = (
    SHARED_DIR
    / 'src/commonTest/kotlin/org/rotki/mobile/core/protocol/generated/'
...
def generated_enum(
    ...
    return '\n'.join((
        f'internal enum class {type_name}(',
...
def generate_vocabulary() -> str:
    ...
    return '\n\n'.join((
        ...
        '\n'.join((
            f'internal const val PROTOCOL_VOCABULARY_SCHEMA_VERSION: Int = {vocabulary["schema_version"]}',
            f'internal val SUPPORTED_PROTOCOL_VERSIONS: Set<Int> = setOf({versions})',
            f'internal val WEBSOCKET_CLOSE_CODES: Set<Int> = setOf({close_codes})',
            f'internal const val DEVICE_PROOF_ALGORITHM: String = "{algorithms[0]}"',
        )),
        '\n'.join((
            'internal object ProtocolHeaders {',
            f'    internal const val IdempotencyKey: String = "{headers["idempotency_key"]}"',
            f'    internal const val Protocol: String = "{headers["protocol"]}"',
            '}',
        )),
        '\n'.join((
            'internal object ProtocolLifetimesSeconds {',
            f'    internal const val AccessSession: Long = {lifetimes["access_session"]}L',
            f'    internal const val Challenge: Long = {lifetimes["challenge"]}L',
            f'    internal const val Pairing: Long = {lifetimes["pairing"]}L',
            '    internal const val ProactiveRenewalWindow: Long = '
            f'{lifetimes["proactive_renewal_window"]}L',
            f'    internal const val RegistrationReplay: Long = {lifetimes["registration_replay"]}L',
            '}',
        )),
        '\n'.join((
            'internal object ProtocolEncodedLengths {',
            f'    internal const val AccessSessionCredential: Int = {lengths["access_session_credential"]}',
            f'    internal const val ChallengeId: Int = {lengths["challenge_id"]}',
            f'    internal const val DeviceSessionId: Int = {lengths["device_session_id"]}',
            f'    internal const val IdempotencyKey: Int = {lengths["idempotency_key"]}',
            f'    internal const val Nonce: Int = {lengths["nonce"]}',
            f'    internal const val PairingCredential: Int = {lengths["pairing_credential"]}',
            f'    internal const val PairingId: Int = {lengths["pairing_id"]}',
            f'    internal const val P1363Signature: Int = {lengths["p1363_signature"]}',
            f'    internal const val P256PublicKey: Int = {lengths["p256_public_key"]}',
            '}',
        )),
        '\n'.join((
            'internal object ProtocolClientInputLimits {',
            '    internal const val MaximumControlResponseBytes: Int = '
            f'{input_limits["maximum_control_response_bytes"]}',
            '    internal const val MaximumJsonNestingDepth: Int = '
            f'{input_limits["maximum_json_nesting_depth"]}',
            '    internal const val MaximumWebSocketBufferedFrames: Int = '
            f'{input_limits["maximum_websocket_buffered_frames"]}',
            '    internal const val MaximumWebSocketEventBytes: Long = '
            f'{input_limits["maximum_websocket_event_bytes"]}L',
            '}',
        )),
        '\n'.join((
            'internal object ProtocolRetryPolicy {',
            '    internal const val SafeGetMaximumAttempts: Int = '
            f'{retry_attempt_limits["safe_get"]}',
            '    internal const val IdempotentWriteMaximumAttempts: Int = '
            f'{retry_attempt_limits["idempotent_write"]}',
            '    internal const val ChallengeOrProofMaximumAttempts: Int = '
            f'{retry_attempt_limits["challenge_or_proof"]}',
            '    internal val RetryableHttpStatuses: Set<Int> = '
            f'setOf({retryable_statuses})',
            '    internal const val BaseDelayMilliseconds: Long = '
            f'{retry_policy["base_delay_milliseconds"]}L',
            '    internal const val MaximumDelayMilliseconds: Long = '
            f'{retry_policy["maximum_delay_milliseconds"]}L',
            '    internal const val MaximumRetryAfterSeconds: Long = '
...
def generate_fixtures() -> str:
    ...
    for constant_name, path in fixture_sources:
        ...
        constants.append(
            f'internal const val {constant_name}: String = """{kotlin_raw_string(raw)}"""',
        )
    return '\n\n'.join((
        '// Generated by shared/tools/generate_companion_protocol_fixtures.py. Do not edit.\n'
        'package org.rotki.mobile.core.protocol.generated',
