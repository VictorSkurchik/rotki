...
internal fun CompanionErrorDto.toDomain(statusCode: Int): CompanionFailure {
    val knownCode = HttpErrorCode.entries.firstOrNull { candidate -> candidate.wireValue == code }
...
private val EXPECTED_ERRORS: Map<HttpErrorCode, ExpectedError> = mapOf(
    ...
    HttpErrorCode.FullClientAuthRequired to expected(
        401,
        false,
        ProtocolErrorAction.AuthenticateFullClient,
        CompanionFailureLocalEffect.UNCHANGED,
    ),
    HttpErrorCode.NotAuthorized to expected(
        401,
        false,
    ...
    HttpErrorCode.PairingUnavailable to expected(
        410,
        false,
        ProtocolErrorAction.PairAgain,
