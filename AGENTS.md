...
```bash
# Run backend server
uv run python -m rotkehlchen --rest-api-port 4242
...
1. **API Communication**: Frontend communicates with backend through one listener (port 4242): REST under `/api/1` and WebSockets under `/ws`
...
- REST or WebSocket connection issues: Check that port 4242 is free; one listener serves
  `/api/1` and `/ws`
